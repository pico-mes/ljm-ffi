 ...
 */
function linuxHelperCheckLJMVersion(libraryName, ljmVersion) {
    ...
    var expectedIndex = checkLJMVersion.length - ljmVersion.length;
    var index = checkLJMVersion.indexOf(ljmVersion);
...
var checkLJMLibForVersion = {
    'linux': function checkLinuxLibraryPath(libLocation, ljmVersion) {
        ...
        if(pathInfo.dir === DEFAULT_LIBRARY_LOC) {
            ...
            // attribute.
            var dirInfo = path.parse(pathInfo);
    ...
    'win32': function checkWin32LibraryPath(libLocation, ljmVersion) {
        ...
        try {
            ...
            wmicOutput = execSync(cmdStr).toString();
        } catch(err) {
            console.log('Error executing string....', cmdStr);
        }

        // Check if we are in the platforms default library path.
        if(pathInfo.dir === DEFAULT_LIBRARY_LOC) {
            // On windows computers we can't determine what version of LJM is
            // installed in the global space without loading it.  
            // We don't need to have any logic to determine 32/64 bit versions.
...
function getLibraryLocation(options) {
    ...
    if(options) {
        ...
        if(typeof(options.ljmVersion) === 'string') {
            ...
            if(isValidStr) {
                var ljmVersionPartials = ljmVersionStr.split(separator);
